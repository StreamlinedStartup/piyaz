...
One ask_user_question batch per turn; use prose only when the answer space is genuinely open-ended, never for a bounded set of options (conventions §5). Depth comes from focus, not coverage.
